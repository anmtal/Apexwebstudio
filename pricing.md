# Pricing — Apex Web Studio

Custom, hand-coded website design for small businesses. Based in Mississauga, Ontario (GTA); serving Canada & the USA. Prices are in CAD; US clients are billed the same figures in USD. All plans are all-inclusive (design, hosting, SSL, security, daily backups, and maintenance).

Last updated: 2026-08-27

## Monthly Plans

Billed month-to-month · $0 upfront · no contract · no minimum term · cancel anytime. CAD and USD figures are identical. Hosting, security, SSL, and monthly maintenance/content updates are included in every plan.

### Landing Page — $149/month
- 1 premium custom page
- Fully responsive design
- Integrated lead form
- Basic SEO optimization
- Secure hosting & SSL
- 2 hours/month content updates

### Growth — $199/month  (most popular)
- Everything in Landing, plus:
- Up to 5 premium pages
- Interactive image/work gallery
- Advanced on-page SEO
- Custom form & CRM integration
- 3 hours/month content updates

### Enterprise — $349/month
- Everything in Growth, plus:
- Up to 10 premium pages
- Custom booking / calendar integration
- Up to 2 custom API integrations
- Dedicated high-performance hosting
- 5 hours/month content updates

### E-Commerce — $559/month
- Everything in Growth, plus:
- Online store (up to 100 products)
- Stripe or Shopify integration
- Automated shipping & taxes
- Secure SSL checkout
- 5 hours/month content updates

## Add-ons
- Brand Identity & Logo Kit — $249 one-time (custom vector logo, typography scales, brand style guide)
- Local SEO & Google Map Pack — $499/month (Google Business Profile setup + management, local citations, Map 3-Pack push, monthly reporting)
- Client CRM & Automation — from $399/month (see the Client CRM & Automation section below)

## Client CRM & Automation

A done-for-you CRM that turns your website into a 24/7 front desk. Billed month-to-month with a $299 one-time setup; cancel anytime. Prices in CAD; US clients are billed the same figures in USD. Message and usage fees (SMS, calls, AI minutes) are billed at cost.

### CRM — Growth — $399/month
- Missed-call text-back
- Automated Google review requests
- Online booking + appointment reminders
- 2-way SMS & WhatsApp inbox
- Lead capture from your website
- Reactivation campaigns + client dashboard

### CRM + AI Receptionist — $679/month  (most popular)
- Everything in Growth, plus:
- 24/7 AI voice receptionist that answers and books calls
- AI chat across SMS, WhatsApp, web & Instagram
- AI replies to Google reviews
- Priority support

### CRM — Multi-Location / Franchise — custom
- Everything in AI Receptionist, plus multiple locations, custom automations & integrations, and dedicated account management

## Notes
- Month-to-month billing with $0 upfront, no contract, and no minimum term — cancel anytime.
- Every website is hand-coded (no page builders or templates) for speed and clean structure.
- Contact: contact@apexwebstudio.ca · +1 (365) 737-1707 · https://www.apexwebstudio.ca
